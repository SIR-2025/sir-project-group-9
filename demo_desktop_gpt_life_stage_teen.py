...
class TeenStage(SICApplication):
    ...
    def setup(self):
        ...
        conf = GPTConf(
            ...
            system_message="You are NAO simulating the TEEN stage.",
            model="gpt-4o-mini",
            temp=0.7,
            max_tokens=80
        )
    ...
    def run(self):
        stage = "teen"
        self.logger.info("Running TEEN stage")

        # --- LOAD TEEN BEHAVIOR ---
        with open("/home/urchin/Documents/Projects/sir-project-group-9/llm_prompts/teen_behaviour.json", "r") as f:
            behavior = json.load(f)

        stage_events = behavior[stage]["events"]

        # --- LOAD ONLY TEEN DESCRIPTION DATA ---
        with open("/home/urchin/Documents/Projects/sir-project-group-9/llm_prompts/description/teen_description.json", "r") as f:
        ...
        for event_name, info in stage_events.items():
            ...
            prompt = template.replace("{{TEEN_DESCRIPTION}}", scene_desc)
...
if __name__ == "__main__":
    TeenStage(env_path=abspath(join("..", "..", "conf", ".env"))).run()
